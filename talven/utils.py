...
"""Type alias used by :py:obj:`talven.utils.get_xpath`,
:py:obj:`talven.utils.eval_xpath` and other XPath selectors."""
...
"""Languages supported by most searxng engines (:py:obj:`talven.sxng_locales.sxng_locales`)."""
...
def extract_url(xpath_results: list[ElementType] | ElementType | str | Number | bool | None, base_url: str) -> str:
    ...
    Example:
        >>> def f(s, search_url):
        >>>    return talven.utils.extract_url(html.fromstring(s), search_url)
        ...
        >>> talven.utils.extract_url([], 'https://example.com')
        raise ValueError
...
def eval_xpath_list(element: ElementType, xpath_spec: XPathSpecType, min_len: int | None = None) -> list[t.Any]:
    """Same as :py:obj:`talven.utils.eval_xpath`, but additionally ensures the
...
def eval_xpath_getindex(
    ...
    """Same as :py:obj:`talven.utils.eval_xpath_list`, but returns item on
    position ``index`` from the list (index starts with ``0``).

    The exceptions known from :py:obj:`talven.utils.eval_xpath` are thrown. If a
...
def detect_language(text: str, threshold: float = 0.3, only_search_languages: bool = False) -> str | None:
    ...
    :param bool only_search_languages: If ``True``, returns only supported
        SearXNG search languages.  see :py:obj:`talven.languages`
    ...
    a. SearXNG's locale of a query is passed to the
       :py:obj:`talven.locales.get_engine_locale` to get a language and/or region
