...
from talven.utils import load_module
from talven.result_types.answer import BaseAnswer


_default = pathlib.Path(__file__).parent
log: logging.Logger = logging.getLogger("talven.answerers")
    ...
    def load_builtins(self):
        ...
        for f in _default.iterdir():
            ...
            if f.is_file() and f.suffix == ".py":
                self.register_by_fqn(f"talven.answerers.{f.stem}.SXNGAnswerer")
                continue
            ...
            if f.is_dir() and (f / "answerer.py").exists():
                warnings.warn(
                    f"answerer module {f} is deprecated / migrate to talven.answerers.Answerer", DeprecationWarning
                )
    ...
    def ask(self, query: str) -> list[BaseAnswer]:
        ...
        for answerer in self[keyword]:
            for answer in answerer.answer(query):
                # In case of *answers* prefix ``answerer:`` is set, see talven.result_types.Result
