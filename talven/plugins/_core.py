...
if t.TYPE_CHECKING:
    from talven.search import SearchWithPlugins
    from talven.result_types import Result, EngineResults, LegacyResult  # pyright: ignore[reportPrivateLocalImportUsage]
    import flask

log: logging.Logger = logging.getLogger("talven.plugins")
...
class Plugin(abc.ABC):
    ...
    def on_result(self, request: SXNG_Request, search: "SearchWithPlugins", result: "Result") -> bool:
        ...
        .. hint::
           ...
           If :py:obj:`Result.url <talven.result_types._base.Result.url>` is modified,
           :py:obj:`Result.parsed_url <talven.result_types._base.Result.parsed_url>` must
    ...
    def post_search(
        ...
        :py:obj:`Result <talven.result_types._base.Result>` objects to be added to the
        final result list."""
        return
...
class PluginStorage:
    ...
    def post_search(self, request: SXNG_Request, search: "SearchWithPlugins") -> None:
        ...
        <talven.results.ResultContainer`> with result items from plugins listed
        ...
        for plugin in [p for p in self.plugin_list if p.id in search.user_plugins]:
            ...
            # In case of *plugins* prefix ``plugin:`` is set, see talven.result_types.Result
