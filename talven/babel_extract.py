...
"""This module implements the :origin:`talven.msg <babel.cfg>` extractor to
extract messages from:

- :origin:`searx/talven.msg`

The ``talven.msg`` files are selected by Babel_, see Babel's configuration in
:origin:`babel.cfg`::

    talven.msg = talven.babel_extract.extract
    ...
    [talven.msg: **/talven.msg]

A ``talven.msg`` file is a python file that is *executed* by the
:py:obj:`extract` function.  Additional ``talven.msg`` files can be added by:

1. Adding a ``talven.msg`` file in one of the SearXNG python packages and
...
talven.msg_FILE = "talven.msg"
_MSG_FILES = [path.join(path.dirname(__file__), talven.msg_FILE)]
...
def extract(
    ...
    """Extract messages from ``talven.msg`` files by a custom extractor_.
