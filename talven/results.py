...
from talven import logger as log
import talven.engines
from talven.metrics import histogram_observe, counter_add
from talven.result_types import Result, LegacyResult, MainResult
from talven.result_types.answer import AnswerSet, BaseAnswer
...
def calculate_score(
    ...
    for result_engine in result['engines']:
        if hasattr(talven.engines.engines.get(result_engine), 'weight'):
            weight *= float(talven.engines.engines[result_engine].weight)
...
class ResultContainer:
    ...
    def extend(
        ...
        if engine_name in talven.engines.engines:
            eng = talven.engines.engines[engine_name]
    ...
    def get_ordered_results(self) -> list[MainResult | LegacyResult]:
        ...
        for res in results:
            ...
            engine = talven.engines.engines.get(res.engine or "")
            if engine:
    ...
    def add_unresponsive_engine(self, engine_name: str, error_type: str, suspended: bool = False):
        with self._lock:
            if self._closed:
                ...
                return
            if talven.engines.engines[engine_name].display_error_messages:
...
def merge_two_infoboxes(origin: LegacyResult, other: LegacyResult):
    ...
    # pylint: disable=too-many-branches
    weight1 = getattr(talven.engines.engines[origin.engine], "weight", 1)
    weight2 = getattr(talven.engines.engines[other.engine], "weight", 1)
