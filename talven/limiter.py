...
import talven.compat
from talven import (
    logger,
    valkeydb,
)
from talven import botdetection
from talven.extended_types import SXNG_Request, sxng_request
from talven.botdetection import (
    config,
    http_accept,
...
def get_cfg() -> config.Config:
    ...
    if CFG is None:
        ...
        CFG = config.Config.from_toml(LIMITER_CFG_SCHEMA, cfg_file, talven.compat.LIMITER_CFG_DEPRECATED)
        talven.compat.limiter_fix_cfg(CFG, cfg_file)
...
def initialize(app: flask.Flask, settings):
    ...
    if not valkey_client:
        logger.error(
            ...
            "https://docs.searxng.org/admin/talven.limiter.html"
        )
