...
includes the configuration of the (:ref:`Talven appl <talven settings.yml>`)
server. The default configuration for the application server is loaded from the
:origin:`DEFAULT_SETTINGS_FILE <talven/settings.yml>`.  This default
configuration can be completely replaced or :ref:`customized individually
<use_default_settings.yml>` and the ``TALVEN_SETTINGS_PATH`` environment
...
- By default, local configurations are expected in folder ``/etc/talven`` from
  where applications can load them with the :py:obj:`get_yaml_cfg` function.

- By default, customized :ref:`Talven appl <talven settings.yml>` settings are
...
def get_user_cfg_folder() -> Path | None:
    ...
    1. If the ``TALVEN_SETTINGS_PATH`` environment is set and points to a
       folder (e.g. ``/etc/mytalven/``), all local configurations are expected in
       this folder.  The settings of the :ref:`Talven appl <talven
       settings.yml>` then expected in ``settings.yml``
       (e.g. ``/etc/mytalven/settings.yml``).

    2. If the ``TALVEN_SETTINGS_PATH`` environment is set and points to a file
       (e.g. ``/etc/mytalven/myinstance.yml``), this file contains the settings of
       the :ref:`Talven appl <talven settings.yml>` and the folder
       (e.g. ``/etc/mytalven/``) is used for all other configurations.

       This type (``TALVEN_SETTINGS_PATH`` points to a file) is suitable for
       use cases in which different profiles of the :ref:`Talven appl <talven
       settings.yml>` are to be managed, such as in test scenarios.

    3. If folder ``/etc/talven`` exists, it is used.

    In case none of the above path exists, ``None`` is returned.  In case of
    environment ``TALVEN_SETTINGS_PATH`` is set, but the (folder or file) does
    not exists, a :py:obj:`EnvironmentError` is raised.

    """

    folder = None
    settings_path = os.environ.get("TALVEN_SETTINGS_PATH")

    # Disable default /etc/talven is intended exclusively for internal testing purposes
    # and is therefore not documented!
    disable_etc = os.environ.get('TALVEN_DISABLE_ETC_SETTINGS', '').lower() in ('1', 'true')
...
def load_settings(load_user_settings: bool = True) -> tuple[SettingsType, str]:
    ...
    settings_yml = os.environ.get("TALVEN_SETTINGS_PATH")
    if settings_yml and Path(settings_yml).is_file():
        # see get_user_cfg_folder() --> TALVEN_SETTINGS_PATH points to a file
        settings_yml = Path(settings_yml).name
    else:
        # see get_user_cfg_folder() --> TALVEN_SETTINGS_PATH points to a folder
