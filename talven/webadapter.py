...
from talven.webutils import VALID_LANGUAGE_CODE
...
from talven.preferences import Preferences, is_locked
from talven.utils import detect_language
...
def parse_pageno(form: Dict[str, str]) -> int:
    ...
    if not pageno_param.isdigit() or int(pageno_param) < 1:
        raise TalvenParameterException('pageno', pageno_param)
...
def parse_lang(preferences: Preferences, form: Dict[str, str], raw_text_query: RawTextQuery) -> str:
    ...
    if not VALID_LANGUAGE_CODE.match(query_lang) and query_lang != 'auto':
        raise TalvenParameterException('language', query_lang)
...
def parse_safesearch(preferences: Preferences, form: Dict[str, str]) -> int:
    ...
    if 'safesearch' in form:
        ...
        if not query_safesearch.isdigit():
            raise TalvenParameterException('safesearch', query_safesearch)
...
def parse_time_range(form: Dict[str, str]) -> Optional[str]:
    ...
    if query_time_range not in (None, 'day', 'week', 'month', 'year'):
        raise TalvenParameterException('time_range', query_time_range)
...
def parse_timeout(form: Dict[str, str], raw_text_query: RawTextQuery) -> Optional[float]:
    ...
    try:
        ...
        raise TalvenParameterException('timeout_limit', timeout_limit) from e
...
def get_search_query_from_webapp(
    ...
    1. instance of :py:obj:`talven.search.SearchQuery`
    2. instance of :py:obj:`talven.query.RawTextQuery`
    3. list of :py:obj:`talven.search.EngineRef` instances
    ...
    <talven.utils.detect_language>`.  If language recognition does not have a
    ...
    if not form.get('q'):
        raise TalvenParameterException('q', '')
