...
# fallback values
# if there is talven.version_frozen module, and it is not possible to get the git tag
...
logger = logging.getLogger("talven")
...
def get_git_version() -> tuple[str, str, str]:
    ...
    try:
        subprocess_run("git diff --quiet -- . ':!talven/settings.yml' ':!utils/brand.env'")
...
try:
    vf = importlib.import_module('talven.version_frozen')
...
# this file is generated automatically by talven/version.py
...
"""
        ...
        # usage: eval "$(python -m talven.version)"
        shell_code = f"""
VERSION_STRING="{VERSION_STRING}"
VERSION_TAG="{VERSION_TAG}"
DOCKER_TAG="{DOCKER_TAG}"
