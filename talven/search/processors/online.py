...
if t.TYPE_CHECKING:
    from talven.search.models import SearchQuery
...
class OnlineProcessor(EngineProcessor):
    ...
    def init_network_in_thread(self, start_time: float, timeout_limit: float):
        ...
        talven.network.set_timeout_for_thread(timeout_limit, start_time=start_time)
        # reset the HTTP total time
        talven.network.reset_time_for_thread()
        # set the network
        talven.network.set_context_network_name(self.engine.name)
    ...
    def _send_http_request(self, params: OnlineParams):
        ...
        if params["method"] == "GET":
            req = talven.network.get
        else:
            req = talven.network.post
            if params["data"]:
    ...
    def search(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        try:
            ...
            self.logger.error("SSLError {}, verify={}".format(e, talven.network.get_network(self.engine.name).verify))
