...
import talven.answerers
import talven.plugins
from talven.engines import load_engines
from talven.external_bang import get_bang_url
from talven.metrics import initialize as initialize_metrics, counter_inc
from talven.network import initialize as initialize_network, check_network_configuration
from talven.results import ResultContainer
from talven.search.checker import initialize as initialize_checker
from talven.search.processors import PROCESSORS
from talven.search.processors.abstract import RequestParams
...
class Search:
    ...
    def search_answerers(self):
        ...
        results = talven.answerers.STORAGE.ask(self.search_query.query)
...
class SearchWithPlugins(Search):
    ...
    def _on_result(self, result):
        return talven.plugins.STORAGE.on_result(self.request, self, result)

    def search(self) -> ResultContainer:

        if talven.plugins.STORAGE.pre_search(self.request, self):
            super().search()

        talven.plugins.STORAGE.post_search(self.request, self)
