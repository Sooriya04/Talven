...
from talven.data import data_dir
...
def get_wikipedia_summary(wikipedia_url, searxng_locale):
    ...
    try:
        response = talven.network.get(wikipedia_rest_url, headers=headers, timeout=10)
...
def get_website_description(url, lang1, lang2=None):
    ...
    try:
        response = talven.network.get(url, headers=headers, timeout=10)
        response.raise_for_status()
...
def initialize():
    ...
    wikipedia_engine = talven.engines.engines['wikipedia']
    ...
    for engine_name, engine in talven.engines.engines.items():
...
def fetch_wikidata_descriptions():
    ...
    talven.network.set_timeout_for_thread(60)
